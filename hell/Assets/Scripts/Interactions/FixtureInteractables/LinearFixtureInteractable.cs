...
public class LinearFixtureInteractable : XRBaseInteractable
{
    ...
    protected override void OnSelectExited(SelectExitEventArgs args)
    {
        if (args.interactor == interactor)
        {
            EndDrag();
            interactor = null;
            if (slider.position == end.position)
            {
                auntVoice.Play();
    ...
    IEnumerator MoveCamera()
    {
        ...
        {
            XRRig.transform.position = Vector3.MoveTowards(XRRig.transform.position, RigDestination.transform.position, speed * Time.deltaTime);
            yield return null;
        }
    }
